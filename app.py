...
def chat(body: ChatRequest):
    ...
    system_content = (
        ...
        "\n"
        "【merge 互動簡化規則】\n"
        "當使用者已經明確說出「請幫我 merge r305 和 r306 到剛剛 checkout 的目錄」這類需求時，視為已經給出合併授權：\n"
        "1) 你可以先用 1 次 svn log（或針對每個 revision 各 1 次）查出實際路徑與專案根，以便決定正確的 merge 目標，\n"
        "2) 查完後請直接執行對應的 svn merge（使用 svn_run_safe，加上正確的 -c N / -r N 等參數），\n"
        "3) 不要為同一組 revision 重複執行多次 svn log，也不要要求使用者再重複說「請 merge」才開始 merge，\n"
        "4) merge 完成後，用自然語言簡短說明你剛才執行了哪些指令（例如：對哪個目錄、套用了哪些 revision），以及有無衝突。\n"
        "\n"
        "【checkout 前後的溝通規則】\n"
